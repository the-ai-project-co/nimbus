...
import { rollbackCommand, type RollbackOptions } from '../../src/commands/rollback';
...
describe('parseTimeoutToMs (L1)', () => {
  it('exists and is importable', async () => {
    const { parseTimeoutToMs } = await import('../../src/commands/rollout');
  ...
  it('rolloutCommand is exported from rollout.ts', async () => {
    const { rolloutCommand } = await import('../../src/commands/rollout');
