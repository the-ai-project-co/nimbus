...
const INCIDENT_SRC = readFileSync(join(__dirname, '../../src', 'commands', 'incident.ts'), 'utf-8');
...
describe('PD_API_TOKEN guard (G14)', () => {
  ...
  it('incidentCommand with plain text does not throw when chatCommand is mocked', async () => {
    ...
    const { incidentCommand } = await import('../../src/commands/incident');
