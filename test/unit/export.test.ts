 ...
 *
 * The export command uses `await import('../../src/sessions/manager')` dynamically,
...
describe('exportCommand no session (G19)', () => {
  it('calls process.exit(1) when no sessions exist', async () => {
    ...
    vi.doMock('../../src/../src/sessions/manager', () => ({
      SessionManager: {
        getInstance: vi.fn(() => ({
          list: vi.fn(() => []),
    ...
    try {
      const { exportCommand: exportCmd } = await import('../../src/commands/export');
