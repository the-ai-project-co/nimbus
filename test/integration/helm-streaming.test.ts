...
const DEVOPS_SRC = readFileSync(join(__dirname, '../../src', 'tools', 'schemas', 'devops.ts'), 'utf-8');
...
describe('helmTool routes to spawnExec for streaming actions (G10)', () => {
  ...
  it('calls spawnExec for upgrade action and returns success', async () => {
    const { spawnExec } = await import('../../src/tools/spawn-exec');
    const { helmTool } = await import('../../src/tools/schemas/devops');
