...
import type { ToolDefinition, ToolExecuteContext } from '../../src/tools/schemas/types';
import type { FileDiffDecision } from '../../src/agent/loop';
import { z } from 'zod';
...
describe('GAP-20 — devops.ts DEFAULT_TIMEOUT constant', () => {
  it('DEFAULT_TIMEOUT constant is defined in devops.ts', async () => {
    const { readFileSync } = await import('node:fs');
    const { join } = await import('node:path');
    const devopsSrc = readFileSync(join(__dirname, '../../src', 'tools', 'schemas', 'devops.ts'), 'utf-8');
...
describe('GAP-20 — ink/index.ts parseToolTimeouts integration', () => {
  it('parseToolTimeouts function is defined in ink/index.ts source', async () => {
    const { readFileSync } = await import('node:fs');
    const { join } = await import('node:path');
    const inkSrc = readFileSync(join(__dirname, '../../src', 'ui', 'ink', 'index.ts'), 'utf-8');
...
describe('parallel read-only tool dispatch (H2)', () => {
  ...
  it('cloudDiscoverTool schema has regions array field', async () => {
    const { devopsTools } = await import('../../src/tools/schemas/devops');
