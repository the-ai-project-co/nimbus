...
import type { ToolCompletionRequest, StreamChunk } from '../../src/llm/types';
...
describe('OpenRouterProvider.streamWithTools', () => {
  test('text-only streaming yields text chunks then a done chunk', async () => {
    ...
    const { OpenRouterProvider } = await import('../../src/llm/providers/openrouter');
...
describe('OpenAICompatibleProvider.streamWithTools', () => {
  async function createProvider() {
    const { OpenAICompatibleProvider } = await import('../../src/llm/providers/openai-compatible');
