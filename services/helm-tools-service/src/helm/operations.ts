...
export class HelmOperations {
  ...
  private async execute(args: string[]): Promise<CommandResult> {
    ...
    try {
      const result = await $`${this.helmPath} ${fullArgs}`.quiet();
      return {
        success: true,
  ...
  async install(options: InstallOptions): Promise<CommandResult> {
    ...
    if (options.values) {
      ...
      const fullArgs = [...baseArgs, ...args];
      try {
        const result = await $`echo ${options.values} | ${this.helmPath} ${fullArgs}`.quiet();
        return {
          success: true,
          output: result.stdout.toString().trim(),
          exitCode: 0,
   ...
   * Lint a chart
   */
  async lint(path: string, strict?: boolean, valuesFiles?: string[]): Promise<CommandResult> {
