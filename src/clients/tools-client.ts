...
export class ToolsClient {
  ...
  git = {
    ...
    revert: async (commit: string, options?: { cwd?: string; noCommit?: boolean; noEdit?: boolean }) => {
      return this.gitClient.post('/api/git/revert', { commit, ...options });
    },

    cherryPick: async (commit: string, options?: { path?: string; noCommit?: boolean; edit?: boolean; signoff?: boolean; strategy?: string }) => {
    ...
    tag: async (name: string, options?: { path?: string; message?: string; annotated?: boolean; force?: boolean; commit?: string }) => {
    ...
    blame: async (file: string, options?: { path?: string; startLine?: number; endLine?: number }) => {
      const params = new URLSearchParams();
      params.set('file', file);
      if (options?.path) params.set('path', options.path);
      if (options?.startLine) params.set('startLine', options.startLine.toString());
      if (options?.endLine) params.set('endLine', options.endLine.toString());
  ...
  terraform = {
    ...
    state: {
      ...
      mv: async (directory: string, source: string, destination: string) => {
        return this.terraformClient.post('/api/terraform/state/mv', { directory, source, destination });
      },
  ...
  k8s = {
    ...
    portForward: async (pod: string, ports: string, options?: { namespace?: string; kubeconfig?: string; context?: string }) => {
      return this.k8sClient.post('/api/k8s/port-forward', { pod, ports, ...options });
    },

    top: async (resourceType: 'pods' | 'nodes', options?: { namespace?: string; kubeconfig?: string; context?: string }) => {
      const params = new URLSearchParams();
      if (options?.namespace) params.set('namespace', options.namespace);
      if (options?.kubeconfig) params.set('kubeconfig', options.kubeconfig);
      if (options?.context) params.set('context', options.context);
    ...
    patch: async (resource: string, name: string, patch: string, options?: { namespace?: string; type?: 'strategic' | 'merge' | 'json'; kubeconfig?: string; context?: string }) => {
      return this.k8sClient.post('/api/k8s/patch', { resource, name, patch, ...options });
    },

    label: async (resource: string, name: string, labels: Record<string, string>, options?: { namespace?: string; overwrite?: boolean; kubeconfig?: string; context?: string }) => {
      return this.k8sClient.post('/api/k8s/label', { resource, name, labels, ...options });
    },

    annotate: async (resource: string, name: string, annotations: Record<string, string>, options?: { namespace?: string; overwrite?: boolean; kubeconfig?: string; context?: string }) => {
