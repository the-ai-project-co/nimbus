...
async function build(target?: string) {
  ...
  try {
    if (target === '--all' || target === 'all') {
      ...
      await $`bun build ${ENTRY} --compile --outfile ${outfile} --external react-devtools-core --external ink --external ink-text-input --external ink-spinner`;
...
async function buildForTarget(target: BuildTarget) {
  ...
  try {
    await $`bun build ${ENTRY} --compile --target ${target.bunTarget} --outfile ${outfile} --external react-devtools-core --external ink --external ink-text-input --external ink-spinner`;
    const stats = fs.statSync(outfile);
